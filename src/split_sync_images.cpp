...
SplitImage::SplitImage() : Node("split_sync_image_node")
{
    ...
    camera_id_ = std::string(this->get_namespace()).substr(1);
    ...
    leftImagePub_ = this->create_publisher<sensor_msgs::msg::Image>("left/image_raw", 1);
    rightImagePub_ = this->create_publisher<sensor_msgs::msg::Image>("right/image_raw", 1);

     // Create publishers for left and right camera info
    leftCameraInfoPub_ = this->create_publisher<sensor_msgs::msg::CameraInfo>("left/camera_info", 1);
    rightCameraInfoPub_ = this->create_publisher<sensor_msgs::msg::CameraInfo>("right/camera_info", 1);

    // Subscribe to synchronized image topic
    syncImageSub_ = this->create_subscription<sensor_msgs::msg::Image>("sync/image_raw", 10, std::bind(&SplitImage::syncImageCallback, this, std::placeholders::_1));
}
...
void SplitImage::syncImageCallback(const sensor_msgs::msg::Image::SharedPtr msg)
{
    ...
    std::string leftFrame = camera_id_ + "_left_optical_frame";
    std::string rightFrame = camera_id_ + "_right_optical_frame";
